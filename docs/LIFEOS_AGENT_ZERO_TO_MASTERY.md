# LifeOS-Agent 从零到熟练：模型、训练、Tool Calling 与 Agent RL 推导主教材
...
```mermaid
flowchart TD
    A["数学基础：张量、概率、log、梯度"] --> B["Tokenizer 与 Chat Template"]
    B --> C["Transformer：Embedding、Attention、MLP"]
    C --> D["Logits、Softmax、Cross Entropy"]
    D --> E["SFT：标准 assistant token"]
    E --> F["DPO：chosen 相对 rejected"]
    D --> G["Policy Gradient：最大化期望 Reward"]
    G --> H["PPO：Critic、GAE、Clip"]
    G --> I["GRPO：同组相对 Advantage"]
    E --> J["Tool Calling：格式与二轮回答"]
    I --> K["Agent RL：多轮环境与 Action Mask"]
    J --> K
    K --> L["3090 Ti 训练、评测、回滚、发布"]
```
...
- 正确类别：$p_k-1<0$，梯度下降会提高正确 logit。
- 错误类别：$p_k-0>0$，梯度下降会降低错误 logit。
...
=-
\frac{1}{\sum_t m_t}
\sum_t m_t\log p_\theta(y_t\mid x_{<t})
$$
...
不要根据“这个 token loss 是正数”判断更新方向。看导数：由于 $c$ 和 $A$ 在该分支被当成权重，$A>0$ 时最小化 loss 会提高这个实际 token 的 log probability。

如果轨迹 4 的 $A<0$，梯度方向相反，会降低其非法 action token 再次出现的概率。
